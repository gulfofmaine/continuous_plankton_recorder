...
dom_species_hw <- dom_spec_hw %>% map_dfr(tidy_dom_species) %>% drop_na()
...
### Fig. 2: Dominant Species Biomass




```{r}
#| label: dominant-species-temp
#| eval: false
...
# 
# # dominant species temperature optima
# # Problem: Size is continuous, not binned
# 
# # Plot design is stacked bars, organized by temperature optima and grouped by body size
# 
# # Add the size bins they fall into based on max size
# # Active Copepods
# dom_cops <- dom_spec_ctrl %>% 
#   filter(functional_group %in% c("Active Copepod", "Passive Copepod")) %>% 
#   mutate(
#     size_bin = case_when(
#       max_size < 10^-2 ~ "1",
#       between(max_size, 10^-2, 10^-1) ~ "2",
#       between(max_size, 10^-1, 10^0) ~ "3",
#       between(max_size, 10^0, 10^1) ~ "4",
#       between(max_size, 10^1, 10^2) ~ "5",
#       max_size >= 10^2 ~ "6"),
#     size_bin = factor(size_bin, level = as.character(c(1:6))),
#     seasons = factor(seasons, levels = season_lvls))
#     
#     
# # And Protists,
# # gotta do their own size bins
# dom_prots <- dom_spec_ctrl %>% 
#   filter(functional_group %in% c("Protist")) %>% 
#   mutate(
#     size_bin = case_when(
#       max_size < 10^-6 ~ "1",
#       between(max_size, 10^-6, 10^-5) ~ "2",
#       between(max_size, 10^-5, 10^-4) ~ "3",
#       between(max_size, 10^-4, 10^-3) ~ "4",
#       between(max_size, 10^-3, 10^-2) ~ "5",
#       max_size >= 10^-2 ~ "6"),
#     size_bin = factor(size_bin, level = as.character(c(1:6))),
#     seasons = factor(seasons, levels = season_lvls))
# 
# 
# # Active Copepod Plot
# f2_right_ac <- dom_cops %>% 
#   filter(functional_group == "Active Copepod") %>% 
#   ggplot(aes(temp_opt, y = absolute_bio)) +
#   geom_col(aes(fill = size_bin), color = "white", linewidth = 0.25, show.legend = F) +
#   scale_fill_manual(
#     values = c(
#       # rcartocolor::carto_pal(6, "ag_Sunset")
#       "1" = "#EDD9A3",
#       "2" = "#F89F77",
#       "3" = "#F7667C",
#       "4" = "#D43F96",
...
#   labs(y = "Biomass", x = "Temperature Preference (\u00b0C)", title = "Dominant Species Biomass")
# 
# 
# # Passive Copepods
# f2_right_pc <- dom_cops %>% 
...
#   scale_x_continuous(breaks = seq(12, 32, 4),limits = c(12, 32)) +
#   facet_wrap(~seasons, nrow = 1, scales = "free_x") +
#   labs(y = "Biomass", x = "Temperature Preference (\u00b0C)")
# 
# 
# # Protist Figure
# f2_right_prots <- dom_prots %>% 
#   filter(functional_group %in% c("Protist")) %>% 
#    ggplot(aes(temp_opt, y = absolute_bio)) +
#   geom_col(aes(fill = size_bin), color = "white", linewidth = 0.25, show.legend = F) +
#   scale_fill_manual(
...
# 
# # Assemble
# fig2_right_all <- f2_right_ac / f2_right_pc / f2_right_prots 
# fig2_right_all
...
f2_right_ac <- dom_temps %>% 
  ...
  #scale_fill_carto_d(palette = "Geyser", direction = -1) +
  scale_fill_manual(values = temp_val_labels) +
  scale_y_continuous(expand = expansion(add = c(0,1))) +
  scale_x_continuous(
    trans = "log10", labels = label_log(base = 10), limits = 10^c(-6,3)) +
  facet_wrap(~seasons, nrow = 1, scales = "free_x") +
  labs(y = NULL, x = NULL, title = "Dominant species biomass", fill = temp_fill_lab)
...
# # All of them
fig2_full <- fig2_left_all | fig2_mid_all | fig2_right_all 
fig2_full
```
...
shannon_plot <- function(fgroup, ptitle){
  ...
  # rect_df <- data.frame(
  #   hw_status = c("Year 0:\npre-heatwave", "Year 1:\nseasonal heatwave", "Years 2-9:\nafter heatwave"),
  #   hw_fill = c("#C7E9C0", "#FDDBC7", "white"),
  #   xmin = c(-.5, 0.5, 1.5),
  #   xmax = c(0.5, 1.5, Inf),
  #   ymin = rep(-Inf, 3),
  #   ymax = rep(Inf, 3)
...
shan_all <- shan_all #+ plot_annotation(tag_levels = list(c("(a)", "(b)", "(c)")))
shan_all
