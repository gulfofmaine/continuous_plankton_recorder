...
season_breaks <- c(1:length(unique(seasonal_response_all$`Autumn HW`$matrix_row)))
...
# Get the label order
lab_order <- seasonal_response_all$`Autumn HW` %>% 
  distinct(matrix_row, seasons) %>% 
  arrange(matrix_row) %>% 
  pull(seasons)


# Plot Horizon Plot Using All Years
seasonal_response_all$`Autumn HW` %>% 
  filter(hw_season != "Control", 
         is.na(seasons) == FALSE) %>% 
  filter(!temp_opt %in% str_c(c(0, 4, 8, 12, 16),"C")) %>% #distinct(seasons)
  filter(str_detect(functional_group,"Copepod")) %>% 
  ggplot() +
  geom_horizon(
    aes(
      x = matrix_row, 
