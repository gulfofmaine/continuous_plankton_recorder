...
.large[Adam A. Kemberling | Quanittative Technician | 20 Nov 2019]
...
sst_long_lagged <- read_csv(here::here("data", "processed_data", "SST_with_lags.csv")) %>% 
...
species_periods_long <- read_csv(here::here("data", "processed_data", "cpr_with_SSTlags.csv"))  %>% 
  mutate(period = case_when(
    period == "annual" ~ "Annual",
    period == "jf" ~ "January - February",
    period == "ma" ~ "March - April",
...
---
class: top
### 2. Project PCA Weights Forward to 2018
...
### SST + Zooplankton PCA Modes - Extended to 2018
